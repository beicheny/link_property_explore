...
def run():
    ...
    traceIDFile = open("intervalIDBaseIntersectionEdge200.txt", "w")
    ...
    while traci.simulation.getMinExpectedNumber() > 0:
        ...
        for (index, edgeID) in enumerate(edgeList):
            ...
            for vehicleID in vehicleIDList:
                (x,y) = traci.vehicle.getPosition(vehicleID)
                j = int(edgeID[2]) - 2
                if math.pow(x - IntersectionPos[j][0], 2) + math.pow(y - IntersectionPos[j][1], 2) <= math.pow(2*transmissionRange,2):
        ...
        for IntersectionsVehNum in IntersectionsVehNums:
            ...
            #currentLineNum = currentLineNum + ';' + str(IntersectionsVehNum)			
